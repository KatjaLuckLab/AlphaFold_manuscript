...
class Predicted_model(calculate_template_independent_metrics.Predicted_model):
    ...
    def calculate_template_dependent_metrics(self):
        ...
        self.RMSD_domain, self.num_align_atoms_domain,self.align_score_domain,self.num_align_resi_domain,self.RMSD_backbone_peptide,self.RMSD_all_atom_peptide = calculate_template_dependent_metrics.process_calculate_mutated_model_RMSD(DMI_structure_inst=self.DMI_structure_inst,predicted_model=model_path)
...
def parse_prediction_name(prediction_name):
    """Parse out different components in the standard name for a minimal DMI prediction (e.g. run50_DEG_COP1_1_5IGO_SDQIVPEY.SDQIVPWG)
    ...
    Returns:
        ...
        pdb_id (str): PDB ID
        sequence_change (str): Change of sequence in the form of unmutated_seq.mutated_seq
    """
    splits = prediction_name.split('_')
    run_id = splits[0]
    dmi_name = '_'.join(splits[1:-2])
    pdb_id = splits[-2]
    sequence_change = splits[-1]
    return run_id, dmi_name, pdb_id, sequence_change
    ...
    for run_id in run_ids.split(','):
        ...
        for file in os.listdir(run_path):
            ...
            if os.path.isdir(file_abs):
                _, dmi_name, pdb_id, sequence_change = parse_prediction_name(file)
...
# python3 /Users/chopyanlee/AlphaFold_benchmark/process_mutated_minimal_DMI.py -run_ids 50 -path_to_run /Volumes/imb-luckgr/projects/dmi_predictor/DMI_AF2_PRS/